...
public class LoginActivity extends BasicActivity implements OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String pwd=user.getPassord(name);
        boolean isFistStart=LocalStorage.getBoolean(LoginActivity.this, "isFirstLogin");            
        AutoLogin(name, pwd, isFistStart);          
    }
  //实现自动登陆，链接服务端验证
	public void AutoLogin(String name, String pwd, boolean isFistStart) {
	...
	private void ConnecteServer(String name,String pwd) {
		// TODO Auto-generated method stub
		String url = "http://10.0.2.2:8080/ServerJsonDemo/JsonServlet";
	...
	private class LoginTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... params) {
			...
			try {
				...
				if (response.getStatusLine().getStatusCode() == 200) {					
					...
					getResult=result.getString("loginsuccess");
		...
		protected void onPostExecute(String result) {
			if("login success".equals(result)){
				...
				startActivity(intent);
				finish();
			}else if("login error".equals(result)){
